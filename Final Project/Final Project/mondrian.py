...
def find_median(partition, dim):
    ...
    frequency = frequency_set(partition, dim)#屬性的donmain出現的次數
    split_val = ''#在哪個值做分割
    next_val = ''#下個要做分割的值
    value_list = list(frequency.keys())#屬性的domain
    value_list.sort(key=cmp_to_key(cmp_value))
    total = sum(frequency.values())#總筆數30162
    middle = total // 2#15081
...
def anonymize_relaxed(partition):
    ...
    dim = choose_dimension(partition)#0
    if dim == -1:
        print("Error: dim=-1")
        pdb.set_trace()
    # use frequency set to get median
    ...
    # Update parent low and high
    if low is not '':
        partition.low[dim] = QI_DICT[dim][low]#0
        partition.high[dim] = QI_DICT[dim][high]#71
    if split_val == '':
        # cannot split
        partition.allow[dim] = 0
        ...
        return
    # split the group from median
    mean = QI_DICT[dim][split_val]#20
    lhs_high = partition.high[:]#[71,6,15,6,13,4,1,40]
    rhs_low = partition.low[:]#[0,0,0,0,0,0,0,0]
    lhs_high[dim] = mean#20
    rhs_low[dim] = QI_DICT[dim][next_val]#21
    lhs = Partition([], partition.low, lhs_high)
    rhs = Partition([], rhs_low, partition.high)
    mid_set = []
    for record in partition.member:
...
def init(data, k, QI_num=-1):#data:30162*9
    """
    reset global variables
    """
    global GL_K, RESULT, QI_LEN, QI_DICT, QI_RANGE, QI_ORDER
    if QI_num <= 0:
        QI_LEN = len(data[0]) - 1#8,代表有幾個attribute
    else:
        QI_LEN = QI_num
    GL_K = k
    RESULT = []
    # static values
    QI_DICT = []#每個domain的值的對照表
    QI_ORDER = []#每個attribute的domain
    QI_RANGE = []#每個attribute的最大值減最小值
    att_values = []#每個attribute的domain
    for i in range(QI_LEN):
        att_values.append(set())#att_values = [set(), set(), set(), set(), set(), set(), set(), set()]
        QI_DICT.append(dict())#QI_DICT = [{}, {}, {}, {}, {}, {}, {}, {}]
    for record in data:
        for i in range(QI_LEN):
...
def mondrian(data, k, relax=False, QI_num=-1):
    ...
    init(data, k, QI_num)
    result = []
    data_size = len(data)#30162
    low = [0] * QI_LEN#QI的最小值
    high = [(len(t) - 1) for t in QI_ORDER]#QI的最大值
...
def lossfunction(k,high,RESULT):
    loss_value=0
    for partition in RESULT:
        for record in partition.member[:]:
            for index in range(QI_LEN):
                tmp=QI_ORDER[index][partition.high[index]]-QI_ORDER[index][partition.low[index]]+1
                if(index != 6):
                    loss_value+=((tmp-1)/(high[index]-1))
    print("loss_value:",loss_value)
    loss_tmp.append(loss_value)
    getlossdifference(loss_tmp)
    return loss_value 

def getlossdifference(loss_tmp):
    difference = []
    for i in range(len(loss_tmp)):
        if(i!=0):
            tmp = loss_tmp[i]-loss_tmp[i-1]
            difference.append(tmp)
    print("連續K值之間loss_value之差值:\n",difference)
